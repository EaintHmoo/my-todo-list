...
export default function Sidebar({ isSidebarOpen, setSidebarOpen }: SidebarProps){
    ...
    return (
        <>
            <div onClick={() => setSidebarOpen(false)} className={`fixed inset-0 bg-black/50 z-30 md:hidden transition-opacity duration-300 ${isSidebarOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} />
            <aside className={`bg-white border-r border-gray-200 flex flex-col flex-shrink-0 fixed h-full w-64 z-40 transition-transform duration-300 ease-in-out md:static md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
